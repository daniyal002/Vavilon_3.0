...
export function TheatersTable() {
  ...
  const { theatersQuery, updateTheaterMutation, deleteTheaterMutation } = useTheaters();
                  ...
                  return (
                    <tr key={theater.id} className="hover:bg-purple-900/20">
                      <td className="px-6 py-4">
                        {editingTheater?.id === theater.id ? (
                          <input
                            ...
                            onChange={(e) =>
                              setEditingTheater({ ...editingTheater, name: e.target.value })
                            }
                            className="w-full px-2 py-1 bg-purple-900/50 border border-purple-700/30 rounded text-purple-100"
                          />
                        ) : (
                          <span className="text-purple-200">{theater.name}</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        {editingTheater?.id === theater.id ? (
                          <select
                            ...
                            onChange={(e) =>
                              setEditingTheater({ ...editingTheater, type: e.target.value as Theater['type'] })
                            }
                            className="w-full px-2 py-1 bg-purple-900/50 border border-purple-700/30 rounded text-purple-100"
                          >
                      ...
                      <td className="px-6 py-4">
                        {editingTheater?.id === theater.id ? (
                          ...
                          (editingTheater.type === 'REGULAR' || editingTheater.type === 'VIP') ? (
                            ...
                            // Для FLEXIBLE нельзя редактировать здесь
                            <span className="text-purple-500 italic">Н/Д</span>
                          )
                        ) : theater.type === 'FLEXIBLE' ? (
                          // Для FLEXIBLE показываем количество рядов из rowLayout.length
                          theater.rowLayout?.length ?? 0
                        ) : (
                          theater.rows ?? '-'
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4 text-purple-200">
                        {theater.type === 'FLEXIBLE'
                          ? totalSeatsFlexible
                          : Number(theater.rows ?? 0) * Number(theater.seatsPerRow ?? 0)}
                      </td>
                          ...
                          {editingTheater?.id === theater.id ? (
                            <>
                              <button
                                ...
                                className="p-1.5 md:p-2 bg-green-600/80 rounded-lg text-white hover:bg-green-500 transition-colors flex items-center gap-1"
                              >
                                <Save size={16} />
                              ...
                              <button
                                ...
                                className="p-1.5 md:p-2 bg-gray-600/80 rounded-lg text-white hover:bg-gray-500 transition-colors flex items-center gap-1"
                              >
                                <X size={16} />
                            ...
                            <>
                              <button
                                ...
                                className="p-1.5 md:p-2 bg-yellow-600/80 rounded-lg text-white hover:bg-yellow-500 transition-colors flex items-center gap-1"
                              >
                                <Pencil size={16} />
                              ...
                              <button
                                ...
                                className="p-1.5 md:p-2 bg-red-600/80 rounded-lg text-white hover:bg-red-500 transition-colors flex items-center gap-1"
                              >
                                <Trash2 size={16} />
