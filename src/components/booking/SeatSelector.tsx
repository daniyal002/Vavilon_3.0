...
return (
  <div className="space-y-4 md:space-y-8">
    ...
    {layoutType === 'VIP' ? (
      ...
      <div className="mx-auto max-w-[340px] space-y-2">
        <div className="text-center text-xs text-purple-400 mb-3">Экран</div>
            ...
            <div key={row.number} className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {Array.from({ length: row.seats }).map((_, seatIndex) => {
                  ...
                  const isBooked = isSeatBooked(row.number, seatNumber);

                  return (
                    <button
                      type="button"
                      key={seatNumber}
                      onClick={() => onSelect(row.number, seatNumber)}
                      disabled={
                        isBooked ||
                        (selectedSeats.length >= maxSeats &&
                          !isSeatSelected(row.number, seatNumber))
                      }
                      className={`w-6 h-6 rounded transition-all duration-200 transform hover:scale-110
                        ${
                          isBooked
